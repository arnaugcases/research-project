...
library Median {
        ...
        returns (uint32)
    {
        ...
        if (len % 2 == 0) {
            ...
            return uint32((median1 + median2) / 2);
        } else {
    ...
    function shortSelectTwo(
        ...
        uint32 k1,
        uint32 k2
    ) private pure returns (uint32 k1th, uint32 k2th) {
    ...
    function quickselect(
        ...
        uint32 hi,
        uint32 k
    ) private pure returns (uint32 kth) {
        require(lo <= k);
        require(k <= hi);
        while (lo < hi) {
    ...
    function quickselectTwo(
        ...
        uint32 hi,
        uint32 k1,
        uint32 k2 // for testing
    ) internal pure returns (uint32 k1th, uint32 k2th) {
        require(k1 < k2);
        require(lo <= k1 && k1 <= hi);
        require(lo <= k2 && k2 <= hi);
     ...
     */
    function copy(uint32[] memory list) private pure returns (uint32[] memory) {
